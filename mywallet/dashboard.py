...
class TelaDashboard(ctk.CTkFrame):
    ...
    def _criar_interface(self):
        ...
        self._cols_lista = (
            {"titulo": "Descrição", "minsize": 180, "weight": 1, "anchor": "w"},
            {"titulo": "Categoria", "minsize": 120, "weight": 0, "anchor": "w"},
            {"titulo": "Data", "minsize": 64, "weight": 0, "anchor": "w"},
            {"titulo": "Valor", "minsize": 110, "weight": 0, "anchor": "e"},
        )

        header_lista = ctk.CTkFrame(
            self.frame_lista, fg_color=COR_FUNDO_ITEM, corner_radius=RAIO_ITEM, height=34
        )
        header_lista.pack(fill="x", padx=ESPACO_SM, pady=(8, 6))
        header_lista.pack_propagate(False)
        for i, col in enumerate(self._cols_lista):
            header_lista.grid_columnconfigure(i, minsize=col["minsize"], weight=col["weight"])
            ...
            ctk.CTkLabel(
                ...
                anchor=col["anchor"],
            ).grid(row=0, column=i, sticky=sticky, padx=(14 if i == 0 else 8, 8), pady=8)

        self.frame_lista_inner = ctk.CTkFrame(self.frame_lista, fg_color="transparent")
        self.frame_lista_inner.pack(fill="x", padx=ESPACO_SM, pady=(0, 8))
    ...
    def _adicionar_item_lista(self, descricao, valor, categoria, tipo, data):
        ...
        item = ctk.CTkFrame(
            self.frame_lista_inner, corner_radius=RAIO_ITEM, height=44, fg_color="transparent"
        )
        ...
        for i, col in enumerate(self._cols_lista):
            item.grid_columnconfigure(i, minsize=col["minsize"], weight=col["weight"])

        cel_desc = ctk.CTkFrame(item, fg_color="transparent")
        cel_desc.grid(row=0, column=0, sticky="ew", padx=(14, 8), pady=8)
        item.bind("<Enter>", _hover_on)
        item.bind("<Leave>", _hover_off)

        indicador = ctk.CTkFrame(cel_desc, width=8, height=8, corner_radius=4, fg_color=cor)
        indicador.pack(side="left", padx=(0, 10))
        indicador.pack_propagate(False)

        ctk.CTkLabel(
            cel_desc, text=descricao, font=ctk.CTkFont(size=14), text_color=COR_TEXTO, anchor="w"
        ).pack(side="left", fill="x", expand=True)

        ctk.CTkLabel(
            item, text=categoria, font=ctk.CTkFont(size=13), text_color=COR_TEXTO_SEC, anchor="w"
        ).grid(row=0, column=1, sticky="w", padx=8, pady=8)

        ctk.CTkLabel(
            item, text=data_fmt, font=ctk.CTkFont(size=13), text_color=COR_TEXTO_MUTED, anchor="w"
        ...
        ctk.CTkLabel(
            ...
            text_color=cor,
            anchor="e",
        ).grid(row=0, column=3, sticky="e", padx=(8, 14), pady=8)
